...
### 2.10.4 実値を列挙する探索クエリ(ポータルで実行)

`Identifiers.Type` の実値一覧は Azure Resource Graph / `az graph` からは取得できません。`RawEntityMetadata` はカスタム推奨事項エディター専用のテーブルで、ARG では `Environment` 列すら解決できません(`Operator_FailedToResolveEntity`)。実値の列挙は次のクエリを**ポータルのカスタム推奨事項作成画面**で実行して確認します。型名は推測せず、この結果か、カスタム推奨事項画面の Categories からコピーします。
...
### 2.10.5 代表的な AWS `Identifiers.Type` と含まれる情報
...
> 上表の型名・`Record` パスは環境やコネクタで変わり得ます。本番登録前に、必ず 2.10.4 の探索クエリと 2.5 の充足率チェックで実データを確認してください。
