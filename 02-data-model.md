...
- `Identifiers.Type` の**実値一覧は Azure Resource Graph / `az graph` からは取得できません**。`RawEntityMetadata` はカスタム推奨事項エディター専用のテーブルで、ARG では `Environment` 列すら解決できません(`Operator_FailedToResolveEntity`)。したがって実値の列挙は、後述の探索クエリを**ポータルのカスタム推奨事項作成画面**で実行して確認します。
